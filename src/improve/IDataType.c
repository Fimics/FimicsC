...
void testStackArea() {
    //我们并不关心值是多少了，因为a是局部变量内存已经被回收，不要返回局部变量的地址
   int *p = myFunc();
   printf("*p %d\n",*p);
   char *s =getString();//
   printf("%s\n",s);
...
int a=10;//全局区 相当于 extern int a=10 外部链接 c语言默认省略extern关键字
//内部链接
static int b=20;//静态区

/**
 * 1.全局静态变量和局部静态变量都存储在静态区，都是程序运行期间都是合法有效的
...
void testStatic(){
    static int c=30;//静态区
    /**
     * 在其他文件使用全局变量，得先声明,表示告诉编译器这个符号是存在的
